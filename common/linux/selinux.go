...
func SelinuxEnabled() bool {
	...
	util.LogDebug.Printf("selinuxenabled returned %d and err=%v", rc, err)
	if rc == 0 && err == nil {
		return true
	}
	util.LogDebug.Print("selinux is NOT enabled")
	return false
}
...
func Chcon(context, path string) error {
	if SelinuxEnabled() {
		util.LogDebug.Printf("Chcon about to change context of %s to %s", path, context)
